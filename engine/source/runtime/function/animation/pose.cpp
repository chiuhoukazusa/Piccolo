    ...
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
            ...
            bone_trans_one.m_position  = 
                Vector3::lerp(bone_trans_one.m_position, bone_trans_two.m_position, cur_weight);
            bone_trans_one.m_scale     = 
                Vector3::lerp(bone_trans_one.m_scale, bone_trans_two.m_scale, cur_weight);
            bone_trans_one.m_rotation  = 
                //Quaternion::nLerp(cur_weight, m_bone_poses[i].m_rotation, pose.m_bone_poses[i].m_rotation, true);
                Quaternion::sLerp(cur_weight, m_bone_poses[i].m_rotation, pose.m_bone_poses[i].m_rotation, true);
        }
    }
}
